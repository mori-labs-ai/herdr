...
{"id":"req_notify_pane","method":"notification.show","params":{"title":"run finished","pane_id":"w1:p2","agent":"claude"}}
```
...
`workspace_id`, `tab_id`, and `pane_id` are optional and target the notification at one part of the session; omitting all of them keeps it app-global. The most specific id wins, and the others are checked against it: an unknown id returns `pane_not_found`, `tab_not_found`, or `workspace_not_found`, and a pane that does not belong to the requested tab or workspace returns `notification_target_mismatch`. A targeted notification carries the canonical workspace, tab, and pane ids to clients, which suppress it while that target is already focused and focus the pane when the toast is activated. `agent` is optional and reports the agent name behind the notification for per-agent sound filtering.
