...
`--pane`, `--tab`, and `--workspace` target the notification instead of showing it for the whole app, so clients can suppress it while that target is already focused and focus it when you activate the toast. The most specific id wins, and an id that does not exist, or a tab or workspace that the pane does not belong to, is rejected. `--agent` reports the agent name behind the notification, which clients use for per-agent sound filtering. Without these flags the notification stays app-global.
