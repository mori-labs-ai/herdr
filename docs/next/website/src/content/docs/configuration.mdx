...
The desktop tab row has its own overrides, so it can be themed without moving `panel_bg`, `accent`, or the overlay text colors everywhere else:
...
Each one is unset by default and falls back to the value the tab row uses today: `panel_bg` for the row background, the panel contrast color for the active tab foreground, `accent` for the active tab background, and the overlay colors for inactive tabs. `tab_active_bold` renders the active tab label bold; custom-named tabs are already bold.
...
Configure an ordered tmux-style status area at either edge of the tab row:
...
tab_bar_left = [
  { type = "text", text = " iris ", fg = "#1e1e2e", bg = "#cba6f7", bold = true },
  { type = "command", command = "~/.config/herdr/branch.sh", ansi = true },
]
tab_bar_left_separator = " "
```

`tab_bar_left` takes the same entries as `tab_bar_right` and renders them at the left edge, before the tabs.
...
Set `ansi = true` on a command entry to keep its colors instead of stripping them. Herdr then reads SGR (`ESC[...m`) sequences in that last output line and renders the styled runs: `0` reset, `1` bold, `22` no bold, `39`/`49` default foreground and background, `30`–`37` and `90`–`97` basic and bright foregrounds, `40`–`47` and `100`–`107` basic and bright backgrounds, `38;5;n`/`48;5;n` for the 256-color palette, and `38;2;r;g;b`/`48;2;r;g;b` for truecolor. Parameters outside that set are ignored, and every other control sequence is still stripped. Palette colors stay palette colors, so they follow the terminal theme. Without `ansi`, nothing changes.

`text` entries take optional `fg` and `bg` colors as `#rrggbb` and `bold = true`. A value that is not a six digit hex color is reported as a config warning and ignored.

Separators appear only between visible entries. Set `tab_bar_right_separator = ""` for direct concatenation. On a narrow tab row, the status areas yield to the tabs and their controls: the left area collapses first, then the right one.
